...
- **CSV quoting (M6 — done for embedded delimiters):** all 5 modules split fields in a quoting-aware way via `common.sh` (`csv_fpat` builds a gawk `FPAT`; `csv-joiner.sh` uses `patsplit()`; `AWK_UNQUOTE` provides `unq()`), so quoted fields containing the delimiter (e.g. `"Smith, John"`) are handled correctly. Still out of scope: embedded **newlines** inside quoted fields (row counting/duplicate detection are line-based), and the joiner always emits comma-CSV regardless of the input delimiter.
